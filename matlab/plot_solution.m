function plot_solution(r_x,r_y,r_theta,r_s,v_traj,v_u,model)
...
figure;
subplot(2,2,1);
stairs(ts(1:end-1),model.k_brake*v_u(1,:));
ylabel('$t_{\textrm{brake}}$','Interpreter','latex','fontsize', 30);
subplot(2,2,2);
stairs(ts(1:end-1),model.k_engine*v_u(2,:));
...
% Forces
F_p_r = model.k_engine*v_u(2,:)-model.k_brake*model.brake_bias*v_u(1,:);
F_t_r = -model.k_r*slip_r*(model.mass*model.weight_bias);
F_p_f = -model.k_brake*(1-model.brake_bias)*v_u(1,:);
